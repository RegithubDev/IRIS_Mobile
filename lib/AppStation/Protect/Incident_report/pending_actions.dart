...
import '../api_call/pendingActionApiCall.dart';
...
class PendingActions extends StatefulWidget {
  const PendingActions({Key? key}) : super(key: key);
...
class _PendingActionsState extends State<PendingActions> {
  ...
  void _firstLoad() async {
    ...
    MySharedPreferences.instance
      ...
      MySharedPreferences.instance
          ...
          .then((session) async {
        populatePendingActionList(session, startIndex, pageIndex);
      });
    });
    if (mounted == true) {
      setState(() {
    ...
    if (_hasNextPage == true &&
      ...
      MySharedPreferences.instance
        ...
        MySharedPreferences.instance
          ...
          PendingActionApiCall obj = PendingActionApiCall(session);
          var response = await obj.callPendingActionAPi(startIndex, pageIndex);
          if (response.body == "[]") {
            if (mounted == true) {
              setState(() {
                _hasNextPage = false;
  ...
  void populatePendingActionList(
      String sessionId, int start_index, int last_index) async {
    PendingActionApiCall obj = PendingActionApiCall(sessionId);
    var response = await obj.callPendingActionAPi(start_index, last_index);
    if (response.body == "[]") {
      return;
    }
    for (Map json in json.decode(response.body)) {
                ...
                Expanded(
                  ...
                  children: [
                    ...
                    Padding(
                        ...
                        child: getPendingActionListView())
                  ],
                )),
  ...
  Widget IRCard(int index, CProtect cProtect) {
    return GestureDetector(
      child: Container(
        key: const ValueKey('pendingActionContainer1'),
  ...
  Widget getPendingActionListView() {
