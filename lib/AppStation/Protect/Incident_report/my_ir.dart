...
import '../api_call/myIrApiCall.dart';
import '../models/CProtect.dart';
import '../sort/myir_sort_popup.dart';
import 'incident_form_view.dart';
import 'incident_history.dart';

class MyIR extends StatefulWidget {
  const MyIR({Key? key}) : super(key: key);

  @override
  State<MyIR> createState() => _MyIRState();
}

class _MyIRState extends State<MyIR> {
  ...
  void _firstLoad() async {
    ...
    MySharedPreferences.instance
      ...
      MySharedPreferences.instance
          ...
          .then((session) async {
        populateMyIRList(session, userid, startIndex, pageIndex);
      });
    });
    if (mounted == true) {
      setState(() {
    ...
    if (_hasNextPage == true &&
      ...
      MySharedPreferences.instance
        ...
        MySharedPreferences.instance
          ...
          MyIRApiCall obj = MyIRApiCall(session, userid);
          var response = await obj.callMyIrAPi(startIndex, pageIndex);
          if (response.body == "[]") {
            if (mounted == true) {
              setState(() {
                _hasNextPage = false;
  ...
  void populateMyIRList(
      String sessionId, String userId, int start_index, int last_index) async {
    MyIRApiCall obj = MyIRApiCall(sessionId, userId);
    var response = await obj.callMyIrAPi(start_index, last_index);
                  ...
                  children: [
                    Padding(
                        ...
                        child: TextField(
                          ...
                          onChanged: (value) {
                            filterMyIrSearchResults(value.toLowerCase());
                          },
  ...
  void filterMyIrSearchResults(String query) {
                  ...
                  Column(
                    children: [
                      CircleAvatar(
                        ...
                        backgroundColor: kReSustainabilityRed,
                        child: Center(
                            child: _approverTypeField(cProtect.approver_type)),
                      ),

                      /* Text(cProtect.approver_type.split("IRL")[1],
  ...
  Widget _approverTypeField(value) {
    return Center(
      child: Container(
        key: const ValueKey('myIrContainer3'),
