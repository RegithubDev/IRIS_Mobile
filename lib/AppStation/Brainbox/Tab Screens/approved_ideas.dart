...
import '../API Call/approved_ideas_api_call.dart';
import '../Models/CBrainBox.dart';
import '../Sort/approved_ideas_sort_popup.dart';
import '../ideas_form_view.dart';
import '../ideas_history.dart';

class ApprovedIdeas extends StatefulWidget {
  const ApprovedIdeas({super.key});

  @override
  State<ApprovedIdeas> createState() => _ApprovedIdeasState();
}

class _ApprovedIdeasState extends State<ApprovedIdeas> {
    ...
    MySharedPreferences.instance
      ...
      MySharedPreferences.instance
        ...
        MySharedPreferences.instance
            ...
            .then((session) async {
          populateApprovedIdeasList(session, userid, startIndex, role);
        });
      });
    });
    if (mounted == true) {
      ...
      MySharedPreferences.instance
        ...
        MySharedPreferences.instance
          ...
          MySharedPreferences.instance
            ...
            ApprovedIdeasApiCall obj = ApprovedIdeasApiCall(session);
            var response =
                await obj.callApprovedIdeasAPi(startIndex, userid, role);
            if (response.body == "[]") {
              if (mounted == true) {
                setState(() {
                  _hasNextPage = false;
  ...
  void populateApprovedIdeasList(
      String sessionId, String userId, int startIndex, String role) async {
    ApprovedIdeasApiCall obj = ApprovedIdeasApiCall(sessionId);
    var response = await obj.callApprovedIdeasAPi(startIndex, userId, role);
    if (response.body == "[]") {
      return;
    }
